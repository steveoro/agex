...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    primary.item( :key_firms_and_teams,
      I18n.t(:firms_and_teams, :scope =>[:agex_action]), '#',
      :if => Proc.new { Netzke::Core.current_user ? Netzke::Core.current_user.can_access( :firms ) : false }
    ) do |sub_nav|
      sub_nav.item( :key_firms,
        t('agex_action.firms'),  firms_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :firms ) : nil )
      )
      sub_nav.item( :key_contacts,
        t('agex_action.contacts'), contacts_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :contacts ) : nil )
      )
      sub_nav.item :key_separator3, '<hr>', '#', :class => 'disabled'
      sub_nav.item( :key_human_resources,
        t('agex_action.human_resources'), human_resources_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :human_resources ) : nil )
      )
    end

    primary.item( :key_accounts_projects_and_invoices,
      t('agex_action.accounts_projects_and_invoices'), '#',
      :if => Proc.new { Netzke::Core.current_user ? Netzke::Core.current_user.can_access( :projects ) : false }
    ) do |sub_nav|
      sub_nav.item( :key_accounts,
        t('agex_action.accounts'),  accounts_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :accounts ) : nil )
      )
      sub_nav.item( :key_account_data_import,
        t('agex_action.account_data_import'),  data_import_accounts_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :accounts ) : nil )
      )
      sub_nav.item :key_separator4, '<hr>', '#', :class => 'disabled'
      sub_nav.item( :key_projects,
        t('agex_action.projects'), projects_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :projects ) : nil )
      )
      sub_nav.item( :key_invoices,
        t('agex_action.invoices'), invoices_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :invoices ) : nil )
      )
      sub_nav.item( :key_invoice_analysis,
        t('agex_action.invoice_analysis'), analysis_invoices_path(),
        :class => ( Netzke::Core.current_user ? Netzke::Core.current_user.get_css_class_to_access( :invoices ) : nil )
      )
    end
